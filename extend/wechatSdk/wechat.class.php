...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    {
        $result = \think\facade\Cache::rm($cachename);
        return $result;
    }
    ...
    public function getJsTicket($appid = '', $jsapi_ticket = '')
    {
        ...
        if ($result) {
            ...
            $this->setCache($authname, $this->jsapi_ticket, $expire - 100);
            return $this->jsapi_ticket;
        }
        return false;
    }
    ...
    public function getJsCardTicket($appid = '', $api_ticket = '')
    {
        ...
        if ($result) {
            ...
            $expire = $json['expires_in'] ? intval($json['expires_in']) - 100 : 3600;
            $this->setCache($authname, $this->api_ticket, $expire - 100);
            return $this->api_ticket;
        }
        return false;
    }
    ...
    {
        return self::OAUTH_PREFIX . self::OAUTH_AUTHORIZE_URL . 'appid=' . $this->appid . '&redirect_uri=' . urlencode($callback) . '&response_type=code&scope=' . $scope . '&state=' . $state . '&connect_redirect=1#wechat_redirect';
    }
